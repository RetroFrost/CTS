...
public static class ContactSheetDetector
{
    ...
    public static IReadOnlyList<PixelRect> Detect(SKBitmap image, Zipack2SeparatorDefinition settings)
    {
        ...
        for (var y = 0; y < image.Height; y++)
        {
            ...
            {
                if (!IsSeparatorPixel(pixels[rowOffset + x], settings)) continue;
                yellowPerColumn[x]++;
                rowCount++;
            }
        ...
        foreach (var row in rows)
        {
            foreach (var column in columns)
            {
                ...
                if (IsMostlySeparator(pixels, image.Width, rect, settings)) continue;
                results.Add(rect);
            }
        }
    ...
    private static bool IsSeparatorPixel(SKColor pixel, Zipack2SeparatorDefinition settings)
    {
        if (pixel.Alpha < 96) return false;
        var tolerance = Math.Clamp(settings.Tolerance, 0, 96);
        return Math.Abs(pixel.Red - settings.Red) <= tolerance
            && Math.Abs(pixel.Green - settings.Green) <= tolerance
            && Math.Abs(pixel.Blue - settings.Blue) <= tolerance;
    }
    ...
    private static bool IsMostlySeparator(SKColor[] pixels, int imageWidth, PixelRect rect, Zipack2SeparatorDefinition settings)
    {
        ...
        for (var y = rect.Y; y < rect.Bottom; y += sampleStepY)
        {
            ...
            {
                sampled++;
                if (IsSeparatorPixel(pixels[rowOffset + x], settings)) separators++;
            }
        }
