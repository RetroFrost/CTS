...
from .image_transform import TransformRenderer, source_only
from .model import Project
from .render import AssetCache
...
class TransformExportWorker(QThread):
    """FFmpeg exporter using the exact same transformed renderer as the preview."""
    ...
    def run(self) -> None:
        ...
        try:
            ...
            for index, card in enumerate(cards, start=1):
                source = source_only(card.image)
                if source and cache.load(source) is None:
                    missing.append(f"Card {index}: {cache.errors.get(source, 'could not load image')}")
            if missing:
            ...
            renderer = TransformRenderer(cache)
            ...
            self.stage_changed.emit("Rendering transformed video frames")
