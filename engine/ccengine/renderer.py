...
class FrameRenderer:
    ...
    def __init__(self) -> None:
        ...
        self._badge_shell_cache: Image.Image | None = None
    ...
    def _draw_card_body_uncached(self, canvas: Image.Image, card: Card, x: float, width: int, height: int) -> None:
        ...
        if has_description:
            image_height = int(height * 0.768)
            title_height = int(height * 0.080)
            desc_top = image_height + title_height
        else:
            title_height = max(34, int(height * 0.105))
            title_height = min(title_height, max(1, height - 1))
            image_height = height - title_height
            desc_top = height
        ...
        if source is None:
            ...
            # Preserve the exact Alpha 20 default path for old projects.
            ...
            # A transformed image is clipped by the whole card, not the model's
            # artwork box. The original box remains the neutral backdrop.
        ...
        draw.rectangle((ix, image_height, ix + width, desc_top), fill=self.theme.title_background)
        if has_description:
            draw.rectangle((ix, desc_top, ix + width, height), fill=self.theme.description_background)
        draw.line((ix, image_height, ix + width, image_height), fill=self.theme.divider, width=3)
        if has_description:
            draw.line((ix, desc_top, ix + width, desc_top), fill=self.theme.divider, width=3)
        draw.line((ix, 0, ix, height), fill=self.theme.divider, width=3)
        draw.line((ix + width - 1, 0, ix + width - 1, height), fill=self.theme.divider, width=3)

        title_font = self._font(max(27, int(width * 0.072)), True, "title")
        title = card.title or "Untitled"
        title_box = draw.textbbox((0, 0), title, font=title_font)
        while getattr(title_font, "size", 22) > 22 and title_box[2] > width - 30:
            title_font = self._font(getattr(title_font, "size", 24) - 2, True, "title")
            title_box = draw.textbbox((0, 0), title, font=title_font)
        draw.text(
    ...
    def _draw_card_body(self, canvas: Image.Image, card: Card, x: float, width: int, height: int) -> None:
        settings = self._active_settings
        key = (
            card.title, card.value, card.description, card.image, width, height,
    ...
    def _badge_shell(self) -> Image.Image:
        if self._badge_shell_cache is not None:
            return self._badge_shell_cache.copy()
        ...
        shadow = Image.new("RGBA", BADGE_SOURCE_SIZE, (0, 0, 0, 0))
        ...
        self._badge_shell_cache = layer.copy()
        return layer
    ...
    def _text_layout(self, card: Card) -> list[tuple[str, float, int]]:
        ...
        # The final line block sits slightly lower than a geometric center;
        # this matches the source badge rather than a generic centered label.
        if len(lines) == 1:
    ...
    def _badge_source(self, card: Card, age: float, *, sticker_entry: bool) -> Image.Image:
        ...
        cache_key = (card.value.upper().strip(), getattr(self._active_settings, "font_badge", "") if self._active_settings else "")
    ...
    def _draw_badge(
        ...
        if not card.value:
            return
        age = self._badge_age(index, seconds, starts)
        if age < BADGE_VISIBLE_START:
            return

        opening_badge = index < 4
        source = self._badge_source(card, age, sticker_entry=opening_badge)
        if age < BADGE_ENTRY_END:
            affine = badge_entry_affine(age) if opening_badge else post_badge_fall_affine(age)
        else:
            scale = self._stage_scale(index, seconds, starts)
            cx, cy = BADGE_CENTER
            affine = (scale, 0.0, 0.0, scale, cx * (1.0 - scale), cy * (1.0 - scale))
    ...
    def _draw_credits_panel(self, canvas: Image.Image, left: float, project: Project) -> None:
        if not project.settings.credits_enabled:
            return
        x = int(round(left))
        if x >= canvas.width or x + REFERENCE_CARD_WIDTH <= 0:
            return

        settings = project.settings
        panel = Image.new("RGB", (REFERENCE_CARD_WIDTH, REFERENCE_HEIGHT), (28, 28, 29))
        draw = ImageDraw.Draw(panel)
        small = self._font(23, False, "credits")
        heading_font = self._font_for_width(settings.credits_heading, 43, 400, bold=True, role="credits", minimum=20)
        item = self._font(23, True, "credits")
        muted = (246, 246, 246)

        top_text = settings.credits_top_text.strip()
        if top_text:
            lines = self._wrapped_lines(draw, top_text, small, 400, 4)
            y = 38
            for line in lines:
                draw.text((REFERENCE_CARD_WIDTH / 2, y), line, font=small, fill=muted, anchor="ma")
                y += 30
        draw.line((56, 210, REFERENCE_CARD_WIDTH - 56, 210), fill=(175, 175, 175), width=2)

        if settings.credits_heading.strip():
            draw.text((REFERENCE_CARD_WIDTH / 2, 286), settings.credits_heading, font=heading_font, fill=(255, 255, 255), anchor="mm")
        ...
        for text, font in rows:
            ...
            draw.text((REFERENCE_CARD_WIDTH / 2, y), text, font=fitted, fill=(255, 255, 255), anchor="mm")
            y += 45 if is_item else 40

        if settings.credits_footer.strip():
            draw.text(
                (REFERENCE_CARD_WIDTH / 2, 985), settings.credits_footer,
                font=self._font(18, True, "credits"), fill=(165, 165, 165), anchor="mm",
            )
        canvas.paste(panel, (x, 0))

    def _draw_end_group(self, canvas: Image.Image, top: float, project: Project) -> None:
        layer = Image.new("RGBA", (REFERENCE_CARD_WIDTH * 3, REFERENCE_HEIGHT), (0, 0, 0, 0))
        draw = ImageDraw.Draw(layer)
        red = (216, 0, 22, 255)
        ...
        margin = 25
        gap = 42
        box_w = (layer.width - margin * 2 - gap) // 2
        box_h = 420
        box_y = 245
        boxes = [
            (margin, box_y, margin + box_w, box_y + box_h, project.settings.end_best_label),
            (margin + box_w + gap, box_y, layer.width - margin, box_y + box_h, project.settings.end_newest_label),
        ]
        for x0, y0, x1, y1, label in boxes:
            draw.rounded_rectangle((x0, y0, x1, y1), radius=12, fill=red)
            draw.text((x0 + 18, y0 + 18), label, font=label_font, fill=(255, 255, 255, 255))

        credit_w, credit_h = 460, 150
        credit_x = (layer.width - credit_w) // 2
        credit_y = 790
        ...
        draw.text((layer.width / 2, credit_y + 35), project.settings.end_credit_label, font=self._font(23, True, "credits"), fill=(255, 255, 255, 255), anchor="mm")
        draw.text(
            (layer.width / 2, credit_y + 91),
    ...
    def _positions_for_cycle(
        ...
        card_w = REFERENCE_CARD_WIDTH
        if index < 4:
            movement = body_progress(local_time)
        else:
            # Cards 5+ form one uninterrupted conveyor. Each cycle advances by
            # exactly one card width over the *entire* cycle, so velocity does
            # not drop to zero while the badge falls or shines. Adjacent cycles
            # meet at identical positions and identical speed.
            movement = clamp(local_time / max(1e-9, cycle_duration))
        if index == 0:
            return {0: lerp(-card_w, 0.0, movement)}
        if index < 4:
            positions = {settled: settled * card_w for settled in range(index)}
            positions[index] = lerp((index - 1) * card_w, index * card_w, movement)
            return positions

        positions: dict[int, float] = {}
        for slot, card_index in enumerate(range(index - 4, index)):
            positions[card_index] = (slot - movement) * card_w
        positions[index] = (4.0 - movement) * card_w
        return positions

    def _credits_x_for_cycle(self, index: int, local_time: float) -> float | None:
        if index == 0:
            return lerp(REFERENCE_WIDTH, REFERENCE_WIDTH - REFERENCE_CARD_WIDTH, body_progress(local_time))
        if index < 3:
            return REFERENCE_WIDTH - REFERENCE_CARD_WIDTH
        if index == 3:
            return lerp(REFERENCE_WIDTH - REFERENCE_CARD_WIDTH, REFERENCE_WIDTH, body_progress(local_time))
        return None
    ...
    def _render_content(
        ...
        positions = self._positions_for_cycle(index, local_time, cycle_duration)

        # Incoming body first, then settled bodies, so the new card reveals from
        # behind the cards already on screen. The badges are composited afterward.
        if index in positions:
            self._draw_card_body(base, project.cards[index], positions[index], REFERENCE_CARD_WIDTH, REFERENCE_HEIGHT)
        for card_index in sorted(positions):
            if card_index == index:
                continue
            self._draw_card_body(base, project.cards[card_index], positions[card_index], REFERENCE_CARD_WIDTH, REFERENCE_HEIGHT)

        for card_index in sorted(positions):
            self._draw_badge(base, project, card_index, positions[card_index], seconds, starts)
        for card_index in sorted(positions):
            self._draw_front_artwork(base, project.cards[card_index], positions[card_index], REFERENCE_CARD_WIDTH, REFERENCE_HEIGHT)

        credits_x = self._credits_x_for_cycle(index, local_time)
        if credits_x is not None:
            # The reference uses the credits panel as a mask while card four enters.
            self._draw_credits_panel(base, credits_x, project)

    def _draw_final_layout(self, canvas: Image.Image, project: Project, seconds: float, starts: list[float]) -> None:
        visible = list(range(max(0, len(project.cards) - 4), len(project.cards)))
        if not visible:
            return
        offset = 4 - len(visible)
        positions = {card_index: (slot + offset) * REFERENCE_CARD_WIDTH for slot, card_index in enumerate(visible)}
        for card_index in visible:
            self._draw_card_body(canvas, project.cards[card_index], positions[card_index], REFERENCE_CARD_WIDTH, REFERENCE_HEIGHT)
        for card_index in visible:
            self._draw_badge(canvas, project, card_index, positions[card_index], seconds, starts)
        for card_index in visible:
            self._draw_front_artwork(canvas, project.cards[card_index], positions[card_index], REFERENCE_CARD_WIDTH, REFERENCE_HEIGHT)
    ...
    def render(self, project: Project, seconds: float, output_size: tuple[int, int] | None = None) -> Image.Image:
        ...
        starts = card_start_times(project)
        segment, progress, segment_start = locate_segment(project, seconds)
        if segment is None:
            return ImageOps.pad(base, output_size, method=Image.Resampling.LANCZOS, color=self.theme.background) if output_size else base
        p = clamp(progress)
        ...
        if segment.kind == "brand_intro":
            ...
            self._draw_final_layout(base, project, final_time, starts)
            wipe = ease_in_out_cubic(p)
            ImageDraw.Draw(base).rectangle((0, 0, REFERENCE_CARD_WIDTH * 3, int(REFERENCE_HEIGHT * wipe)), fill=(0, 0, 0))

        elif segment.kind in {"end_rise", "end_hold", "fade"}:
            final_time = content_duration(project)
            self._draw_final_layout(base, project, final_time, starts)
            ImageDraw.Draw(base).rectangle((0, 0, REFERENCE_CARD_WIDTH * 3, REFERENCE_HEIGHT), fill=(0, 0, 0))
            rise = ease_out_cubic(p) if segment.kind == "end_rise" else 1.0
            self._draw_end_group(base, lerp(REFERENCE_HEIGHT, 0.0, rise), project)
