...
namespace {
constexpr wchar_t kMainClass[] = L"CubicalCreateMainWindow";
constexpr wchar_t kControlsClass[] = L"CubicalCreateControlsWindow";
...
enum Id {
    ...
    ID_PLAY, ID_RESTART, ID_SEEK, ID_TIME_LABEL, ID_STATUS, ID_PREVIEW_BITMAP, ID_MUSIC, ID_LENGTH,
...
fs::path persistent_asset_directory(const char* role) {
    ...
    fs::path output = root / "Cubical Create" / "Imported Assets" / (std::string(role) + "-" + std::to_string(ticks));
...
void start_preview_render(AppState* s, double requested_time) {
    ...
    const auto output = cubical::temporary_path("cubical-create-preview", ".bmp");
...
void open_controls(AppState* s,int initial_tab=0){auto* c=new ControlsState();c->app=s;HWND w=CreateWindowExW(WS_EX_DLGMODALFRAME,kControlsClass,L"Cubical Create — Project Controls",WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_VISIBLE,CW_USEDEFAULT,CW_USEDEFAULT,810,680,s->window,nullptr,s->instance,c);EnableWindow(s->window,FALSE);ShowWindow(w,SW_SHOW);UpdateWindow(w);if(IsWindow(w)){TabCtrl_SetCurSel(c->tabs,std::clamp(initial_tab,0,3));show_page(c,std::clamp(initial_tab,0,3));}MSG m{};while(IsWindow(w)&&GetMessageW(&m,nullptr,0,0)>0){if(!IsDialogMessageW(w,&m)){TranslateMessage(&m);DispatchMessageW(&m);}}EnableWindow(s->window,TRUE);SetForegroundWindow(s->window);}

LRESULT CALLBACK MainProc(HWND hwnd,UINT msg,WPARAM wp,LPARAM lp){auto* s=reinterpret_cast<AppState*>(GetWindowLongPtrW(hwnd,GWLP_USERDATA));if(msg==WM_CREATE){auto* cs=reinterpret_cast<CREATESTRUCTW*>(lp);s=static_cast<AppState*>(cs->lpCreateParams);SetWindowLongPtrW(hwnd,GWLP_USERDATA,reinterpret_cast<LONG_PTR>(s));s->window=hwnd;int x=12;struct B{const wchar_t* t;int id;int w;}bs[]={{L"New",ID_NEW,55},{L"Open",ID_OPEN,60},{L"Save",ID_SAVE,60},{L"Click to Insert Data",ID_INSERT,145},{L"Image Sheet",ID_SHEET,90},{L"Music",ID_MUSIC,68},{L"Length",ID_LENGTH,70},{L"Manual editor",ID_CONTROLS,105},{L"Export MP4",ID_EXPORT,95}};for(auto&b:bs){make_control(s,WC_BUTTONW,b.t,b.id==ID_INSERT?BS_DEFPUSHBUTTON:0,x,10,b.w,30,hwnd,b.id);x+=b.w+6;}
