...
new_install = '''    fun install(candidate: RendererCandidate): InstalledRenderer {
        ...
        staged?.delete()
        return InstalledRenderer(candidate.spec, destination, activeSha == installedSha)
    }
'''
...
''',
    ...
    fun discard(candidate: RendererCandidate?) {
        candidate?.stagedFile?.takeIf(File::isFile)?.delete()
    }
''',
    ...
    fun rendererV3ZipImportIsDiskBackedAndSidecarsAreLazy() {
        ...
        val zipBytes = ByteArrayOutputStream().use { output ->
            ZipOutputStream(output).use { zip ->
                ...
                repeat(1024) { zip.write(ByteArray(1024) { 0x5a.toByte() }) }
                zip.closeEntry()
            }
            output.toByteArray()
        }
        val candidate = store.inspect(ByteArrayInputStream(zipBytes))
