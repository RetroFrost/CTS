...
"""Generate the Python and Kotlin CTS contract adapters from one JSON source.

The JSON contract is the editable source of truth. Run this script after changing it.
Use --check in CI to reject Android-only or desktop-only drift.
"""
...
def render_python(spec: dict[str, Any]) -> str:
    ...
    timing = spec["timing"]
    layout = spec["layout"]
    cards = spec["sample_cards"]
    legacy = ", ".join(py_string(value) for value in model["legacy_ids"])
    fields = ", ".join(py_string(value) for value in spec["fields"])
    colors = "\n".join(
        f"    {py_string(name)}: {py_string(value)}," for name, value in spec["colors"].items()
    )
    samples = "\n".join(
        "    SharedCard({primary}, {secondary}, {title}, {description}),".format(
            primary=py_string(card["badge_primary"]),
            secondary=py_string(card["badge_secondary"]),
            title=py_string(card["title"]),
            description=py_string(card["description"]),
        )
        for card in cards
    )
...
MODEL_ID = {py_string(model["id"])}
MODEL_LABEL = {py_string(model["label"])}
...
def render_kotlin(spec: dict[str, Any]) -> str:
    ...
    samples = "\n".join(
        ...
        description = {description},
            ...
            primary=kt_string(card["badge_primary"]),
            secondary=kt_string(card["badge_secondary"]),
            title=kt_string(card["title"]),
            description=kt_string(card["description"]),
        )
        for card in cards
    )
...
object SharedContract {{
    ...
    const val MODEL_ID = {kt_string(model["id"])}
    const val MODEL_LABEL = {kt_string(model["label"])}
...
def semantic_check(spec: dict[str, Any]) -> list[str]:
    ...
    expected_python = {
        ...
        "REVEAL_SECONDS": timing["reveal_seconds"],
        "SCROLL_SECONDS": timing["scroll_seconds"],
        "END_HOLD_SECONDS": timing["end_hold_seconds"],
        "FADE_SECONDS": timing["fade_seconds"],
        "BODY_WIPE_SECONDS": timing["body_wipe_seconds"],
        "BADGE_DELAY_SECONDS": timing["badge_delay_seconds"],
        "BADGE_SETTLE_SECONDS": timing["badge_settle_seconds"],
        "INTRO_TAIL_HOLD_SECONDS": timing["intro_tail_hold_seconds"],
    }
    ...
    expected_kotlin = {
        ...
        "MODEL_ID": kt_string(model["id"]),
        "MODEL_LABEL": kt_string(model["label"]),
        "VISIBLE_CARDS": str(model["visible_cards"]),
    }
    for name, expected in expected_kotlin.items():
    ...
    for card in spec["sample_cards"]:
        if kt_string(card["title"]) not in kotlin or py_string(card["title"]) not in PYTHON_PATH.read_text(encoding="utf-8"):
            errors.append(f"sample card {card['title']!r} is missing from a generated adapter")
    return errors


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--check", action="store_true", help="fail when generated adapters drift")
