...
text = insert_into_set(
    ...
    "    val features = setOf(",
    [
        "per-frame-polygon-vertices", "full-2d-transforms", "shine-geometry-tracks",
        "arbitrary-masks", "track-interpolation-modes", "per-item-animation",
        "exact-text-tracks", "source-baked-text-raster", "explicit-layer-order",
        "independent-shadow-resources", "independent-shadow-resource", "dense-frame-data",
        "raw-frame-tracks", "frame-addressed-objects", "frame-addressed-selectors",
        "property-level-selector-inheritance", "deterministic-selector-precedence",
        "zero-implicit-animation", "generic-renderer-resources", "group-transforms",
        "resource-lifespans", "selector-shared-behaviour", "renderer-owned-geometry",
        "renderer-owned-materials", "blend-compositing-modes", "per-frame-filter-tracks",
        "exact-artwork-transforms", "absolute-integer-frame-clock",
        "single-scene-preview-export-contract", "preview-export-identical-path",
        "reference-resolution-fps-lock", "frame-checkpoints", "pixel-diff-audit-contract",
        "selector-cascade-inspection", "exact-outro-overlay", "renderer-api-v3-scene-ir",
        "renderer-v3-sidecar-resources", "renderer-v3-zip-package",
    ],
...
if wrapper not in text:
    ...
    text = text.replace(legacy_private, wrapper + legacy_private, 1)
...
if '"scene-v3" -> "scene-v3"' not in text:
    text = replace_once(text, '        "native-standard" -> "native-standard"\n',
...
if '"scene-v3" -> RendererV3Runtime.scene(spec)?.timeline?.frames' not in text:
    ...
    text = replace_once(text, anchor,
                        anchor + '            "scene-v3" -> RendererV3Runtime.scene(spec)?.timeline?.frames ?: spec.canonicalFrameCount\n',
                        "renderer v3 timeline frame count")
...
# Do not replay the constant insertion when modern source already contains it.
if 'const val RENDERER_V3_MIME' not in text:
    text = text.replace(
