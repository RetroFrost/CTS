...
object ReferenceBadgePainter {
    ...
    private fun drawShine(canvas: Canvas, clip: Path, age: Float) {
        val raw = (age - SHINE_START) / SHINE_SECONDS
        if (raw <= 0f || raw >= 1f) return
        val progress = smoothStep(raw); val sourceLeft = 0f; val sourceTop = 0f; val sourceWidth = 480f; val sourceHeight = 430f
        ...
        canvas.save(); canvas.clipPath(clip); fill.shader = null; val broad = band(40f / 480f, 48, maxOf(2f, sourceWidth * 12f / 480f)); canvas.drawPath(broad, fill); val core = band(5f / 480f, 82, maxOf(1f, sourceWidth * 2.8f / 480f)); canvas.drawPath(core, fill); fill.maskFilter = null; canvas.restore()
    }
