...
object CtsMp4Exporter {
    ...
    suspend fun export(
        ...
        val temporaryFile = File.createTempFile("cts-export-", ".mp4", context.cacheDir)
        val renderer = CtsCanvasSceneRenderer(context.applicationContext, normalized)
        ...
        try {
            ...
            codec.start()
            inputSurface = CodecInputSurface(codecSurface)
            muxer = MediaMuxer(
            ...
            for (frameIndex in 0 until frameCount) {
                ...
                val timeSeconds = frameIndex.toFloat() / FRAME_RATE
                renderer.drawFrame(canvas, preset.width, preset.height, timeSeconds)

                inputSurface.makeCurrent()
                inputSurface.draw(bitmap, preset.width, preset.height)
                inputSurface.setPresentationTime(
                    frameIndex.toLong() * 1_000_000_000L / FRAME_RATE,
                )
                check(inputSurface.swapBuffers()) { "Could not submit frame to video encoder." }
